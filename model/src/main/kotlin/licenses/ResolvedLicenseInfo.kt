...
data class ResolvedLicense(
    ...
    init {
        require(sources.isNotEmpty()) {
            "A resolved license must correspond to at least one license source."
        }
    }
    ...
    fun filterExcludedOriginalExpressions(): ResolvedLicense? {
        ...
        val originalExpression = originalExpressions.filterNotTo(mutableSetOf()) { it.isDetectedExcluded }
        if (originalExpression.isEmpty()) return null

        return copy(originalExpressions = originalExpressions)
    }
}
